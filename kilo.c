...
char *C_HL_extensions[] = { ".c", ".h", ".ccp", ".md", ".txt", NULL};
char *C_HL_keywords[] = {
...
void editorProcessKeypress() {
    ...
    switch (c) {
        ...
        case CTRL_KEY('x'): //using x instead of q due to vs code
            if (E.dirty && quit_times > 0) {editorSetStatusMessage("WARNING!!! File has unsaved changes. Press Ctrl-X %d more times to quit.", quit_times);
                quit_times--;
                return;
            }
            write(STDOUT_FILENO, "\x1b[2J", 4);
        ...
        case CTRL_KEY('h'):
            editorDelChar();
            break; //modified to separate out BACKSPACE and DEL_KEY

        case DEL_KEY:
            if (c == DEL_KEY) {
                if (E.cx == E.row[E.cy].size && E.cy == E.numrows - 1) break; //prevents arrow right when at tend of line and bottom
